...
0 * * * * cd /path/to/tomos-memory-stack && \
  ...
  python scripts/memory_discord_alert.py >> /var/log/tomos-health.log 2>&1
```
...
0 * * * * cd /path/to/tomos-memory-stack && \
  DISCORD_MEMORY_WEBHOOK_URL="https://..." \
  python scripts/memory_discord_alert.py --always-post >> /var/log/tomos-health.log 2>&1
```
